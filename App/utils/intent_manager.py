...
shell_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(this_file_path))), "external", "MEC_gateway", "shell_templates")


def intent_translate(user_text):
    intent_dic = text_to_intention(user_text)
    if intent_dic is None:
        return "意图获取失败！"
    intent = intent_dic["intent"]
    if intent == "get_net_info":
        shell_path = os.path.normpath(os.path.join(shell_dir, "get_agent_info.sh"))
        with open(shell_path, "r", encoding="utf-8") as file:
            content = file.read()
            return content
    if intent == "config":
        shell_path = os.path.normpath(os.path.join(shell_dir, ""))
